...
| [E3.1](rig_a/experiments/e3_1_transfer_ranking.py) | net-transfer ranking accumulates abstractions | **PARTIAL** — margin *grows* under noise (+0.147, artifact-free) but inverts to **−0.008** when patches carry real off-target spillover. §B is conditionally true on a condition the design never states. Clean-regime numbers contaminated — see E3.1b |
| [E3.1b](rig_a/experiments/e3_1b_tiebreak_audit.py) | is E3.1's compositional-only figure real? | **FAIL** — patches, comp-only skills and already-learned skills all score exactly 0.000 under net transfer, so pool emission order decides. comp_only spans **0.005–0.780** across tiebreak rules. The published 55%/72% pair is **withdrawn**; clean-regime margins are contaminated too, noisy ones are not |
