...
def main():
    ...
    repeated_logits={};duplicate_queries=0
    ...
    for cell in data['results']:
        ...
        for i,(r,q,view) in enumerate(zip(cell['rows'],d['queries'],d['views'][encoding])):
            ...
            assert r['tokens']==cache[prompt]
            key=(seed,state,prompt)
            if key in repeated_logits:assert repeated_logits[key]==r['logits'];duplicate_queries+=1
            else:repeated_logits[key]=r['logits']
        ...
        unique=[];unique_seen=set()
        for r,q in zip(cell['rows'],d['queries']):
            key=(q['prompt'],q['teacher_index'])
            if r['split']=='tutorial' and key not in unique_seen:unique.append(r);unique_seen.add(key)
        subsets['tutorial_unique_prompt_label']=unique
    ...
    for i,left in enumerate(seeds):
        for right in seeds[i+1:]:
            lp={(q['prompt'],q['teacher_index']) for q in datasets[left]['queries'] if q['split']=='tutorial'};rp={(q['prompt'],q['teacher_index']) for q in datasets[right]['queries'] if q['split']=='tutorial'}
            lt={(left+700000+j,scfg['bootstrap_world_size']) for j in range(scfg['bootstrap_worlds'])};rt={(right+700000+j,scfg['bootstrap_world_size']) for j in range(scfg['bootstrap_worlds'])}
            lf={(left+cfg['fresh_world_seed_offset']+j,n) for j,n in enumerate(cfg['fresh_world_sizes'])};rf={(right+cfg['fresh_world_seed_offset']+j,n) for j,n in enumerate(cfg['fresh_world_sizes'])}
            assert not (lt|rt)&(lf|rf)
            overlap.append(dict(seeds=[left,right],shared_tutorial_world_specs=len(lt&rt),tutorial_worlds_each=len(lt),shared_fresh_world_specs=len(lf&rf),fresh_worlds_each=len(lf),shared_tutorial_prompt_labels=len(lp&rp),unique_tutorial_prompt_labels_each=[len(lp),len(rp)],tutorial_fresh_world_specs_disjoint=True))
    assert sha(path)==before and sha(a.parent/'complete.json')==manifest['source_complete_sha256'];result=dict(status='E31_PREFLIGHT_AUDITED' if a.preflight else 'E31_AUDITED',source_sha256=before,source_commit=manifest['git_commit'],audit_script_sha256=sha(__file__),inventory=inventory,cross_seed_overlap=overlap,cells=allsummary,paired_renaming=pairs,duplicate_identical_prompt_queries_exact=duplicate_queries,total_queries=sum(c['work']['inference_calls'] for c in data['results']),total_tokens=sum(c['work']['inference_tokens'] for c in data['results']),total_attention_elements=sum(c['work']['inference_attention_elements'] for c in data['results']),total_query_seconds=sum(c['query_seconds'] for c in data['results']),model_load_seconds_once_per_seed=sum(next(c['model_load_seconds'] for c in data['results'] if c['seed']==s) for s in cfg['source_seeds']),audit_seconds=time.perf_counter()-start,limits='Static teacher-induced states and correlated repeated queries. All reference sets independently match Floyd-Warshall observed-map distances, metrics recompute from recorded logits, and source adapters match. Neural logits are not independently reexecuted here. Source bootstrap and model pretraining costs are not erased; query-local encodings are not a deployed memory system.');a.out.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps({k:result[k] for k in ('status','total_queries','total_tokens','audit_seconds')}))
