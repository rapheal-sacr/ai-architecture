...
def audit(source,model,preflight=False):
    ...
    for seed in cfg['seeds']:
        ...
        full=next(c for c in group if c['arm']=='frozen_full');bounded=next(c for c in group if c['arm']=='frozen_bounded')
        if not preflight:assert [e['events'] for e in full['episodes'][:4]]==[e['events'] for e in bounded['episodes'][:4]]
        pair_diagnostics.append(dict(seed=seed,full_bounded_initial_four_exact=None if preflight else True,mission_event_sequences_equal=[x['events']==y['events'] for x,y in zip(full['episodes'],bounded['episodes'])],mission_prompt_sequences_equal=[[e['prompt'] for e in x['trajectory']]==[e['prompt'] for e in y['trajectory']] for x,y in zip(full['episodes'],bounded['episodes'])]))
    assert sha(path)==source_hash
    return dict(status='E30_PREFLIGHT_AUDITED' if preflight else 'E30_AUDITED',source_sha256=source_hash,source_commit=data['manifest']['git_commit'],audit_script_sha256=sha(__file__),bootstrap_audits=boot_audits,pair_diagnostics=pair_diagnostics,mission_checkpoints_audited=checkpoints,rows=rows,audit_seconds=time.perf_counter()-start,tokenized_distinct_prompts=len(token_cache),limits='Trajectory and state reconstruction includes actual chosen actions, memory, reservoir, private RNG progression, update selections and token counts. It does not recompute alternate-action logits or neural gradients; scorer supplied base-hash and final restore checks. Full Python overhead, base buffers, energy and pretraining cost are not measured. Attention elements are forward attention sizes, not FLOPs. Bootstrap and audit work are separate from operational use.')
